...
class MetadataService:
    ...
    def _write_image_metadata(self, memory: Memory, file_path: Path, jpeg_quality: int = 95) -> None:
        with Image.open(file_path) as image:
            ...
            exif_data_dictionary["Exif"][piexif.ExifIFD.DateTimeOriginal] = memory.exif_datetime.encode(
                'utf-8')
            exif_data_dictionary["Exif"][piexif.ExifIFD.DateTimeDigitized] = memory.exif_datetime.encode(
                'utf-8')
            exif_data_dictionary["0th"][piexif.ImageIFD.DateTime] = memory.exif_datetime.encode(
                'utf-8')

            coordinates = memory.location_coords
            if coordinates:
                latitude, longitude = coordinates
                latitude_degrees_minutes_seconds = self._decimal_to_dms(
                    latitude)
                longitude_degrees_minutes_seconds = self._decimal_to_dms(
                    longitude)
    ...
    def _write_video_metadata(self, memory: Memory, file_path: Path, ffmpeg_timeout: int) -> None:
        ...
        if subprocess_execution_result.returncode == 0:
            ...
            raise Exception(
                f"ffmpeg failed with code {subprocess_execution_result.returncode}")
