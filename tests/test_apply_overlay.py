    ...
    with patch.object(Config, "cli_options", cli_options):
        ...
        with patch(
            ...
            with patch(
                ...
                with patch(
                    "src.media_dispatcher.zip_processor.ZipProcessor.run",
                ) as mock_zip_run:

                    def zip_run_side_effect():
                        if apply_overlay:
                            mock_apply_overlay(
                                b"data", b"overlay", ".jpg", Path("/fake/path.jpg"),
                            )
